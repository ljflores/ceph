...
class MgrModule(ceph_module.BaseMgrModule, MgrModuleLoggingMixin):
    ...
    def get_perf_histograms(self,
                            svc_type: str,
                            svc_name: str) -> Dict[str,
                                                   Dict[str, Dict[str, Union[str, int]]]]:

        """
        Called by the plugin to fetch perf counter histogram info.
        """
        return self._ceph_get_perf_histograms(svc_type, svc_name)
    ...
    @profile_method()
    def get_all_perf_histograms(self, prio_limit: int = PRIO_USEFUL,
                              services: Sequence[str] = ("mds", "mon", "osd",
                                                         "rbd-mirror", "rgw",
                                                         "tcmu-runner")) -> Dict[str, dict]:
        """
        ...
        for server in self.list_servers():
            for service in cast(List[ServiceInfoT], server['services']):
                ...
                schemas = self.get_perf_histograms(service['type'], service['id'])
                if not schemas:
                    self.log.warning("No perf counter schema for {0}.{1}".format(
                        service['type'], service['id']
                    ))
